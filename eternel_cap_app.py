...
def resize_image(input_path, output_path, max_dimension=2560):
    ...
    with Image.open(input_path) as img:
        ...
        # Resize the image if its dimensions exceed max_dimension
        if max(img.width, img.height) > max_dimension:
            scale_factor = max_dimension / max(img.width, img.height)
            new_width = int(img.width * scale_factor)
            new_height = int(img.height * scale_factor)
            img = img.resize((new_width, new_height), Image.LANCZOS)
            print(f"Resized {os.path.basename(input_path)}: {original_size} -> {img.size}")

        # Save the resized image in the same format, keeping high quality
        img.save(output_path, format=original_format, quality=95, optimize=True)

def process_images_2(brut_folder, compressed_folder, name_prefix):
...
def open_resize_window():
    ...
    root.withdraw()
    resize_window = Tk()
    resize_window.title("Redimension de photos")
    apply_geometry(resize_window)
    resize_window.deiconify()


    # Create a frame for layout
    frame = Frame(resize_window)
    ...
    def select_folder():
        ...
        brut_folder = filedialog.askdirectory(title="Select Brut Files Folder",initialdir='/home/ocr8n/Pictures')
    ...
    def select_folder_compress():
        ...
        compress_folder = filedialog.askdirectory(title="Select Compressed Files Folder",initialdir='/home/ocr8n/Pictures')
    ...
    def run_compression():
        ...
        process_images_2(brut_folder, compress_folder, name_prefix)
    ...
    run_button = Button(button_frame, text="Resize", state="normal", command=run_compression, width=10, height=1)
...
def compress_image(input_path, output_path, max_size_mb=128, quality=85, max_dimension=2560):
    # Open the image
    with Image.open(input_path) as img:
...
def add_logo_to_image(image_path, logo_path, output_path, opacity=80):
    ...
    if img_width > img_height:
        ...
        max_logo_width = img_width // 18
        max_logo_height = img_height // 18
        ...
        # Resize the logo to fit proportionally in the image
        max_logo_width = img_width // 11 
        max_logo_height = img_height // 11
        logo.thumbnail((max_logo_width, max_logo_height))
...
def open_logo_window():
    ...
    def select_folder():
        ...
        # brut_folder = custom_select_folder(initial_dir=os.getcwd(), title="Dossier contenant les photos à signer",geometry=current_geometry)
    ...
    def add_logo_to_folder():    
        ...
        logo_path = 'logos/signature2.png'
...
Radiobutton(root, text="Compresse", variable=selected_option, value="compression").pack(anchor="w")
Radiobutton(root, text="Resize", variable=selected_option, value="resize").pack(anchor="w")
Radiobutton(root, text="Sign", variable=selected_option, value="logo").pack(anchor="w")
Radiobutton(root, text="Filigrane", variable=selected_option, value="watermark").pack(anchor="w")
